...
SUMMARY_PATH = Path("super_adversarial_optuna_summary.json")
BASE_SUBMISSION_PATH = Path("submission_super_adv_optuna_base.csv")
ALPHA_010_SUBMISSION_PATH = Path("submission_super_adv_optuna_alpha_010.csv")
ALPHA_020_SUBMISSION_PATH = Path("submission_super_adv_optuna_alpha_020.csv")
...
def compute_raw_odds_weights(data: dict) -> tuple[np.ndarray, np.ndarray, dict]:
    ...
    X_adv_train = prepare_catboost_frame(X_adv_train, cat_features)
    X_adv_valid = prepare_catboost_frame(X_adv_valid, cat_features)
    X_cv_for_adv = prepare_catboost_frame(data["X_cv_base"], cat_features)
    X_private_for_adv = prepare_catboost_frame(data["X_private_base"], cat_features)
    ...
    raw_odds = local_prob / (1.0 - local_prob)
    ...
    summary = {
        ...
        "weight_formula": "sample_weight = p / (1 - p), clipped at the 99th percentile",
    }
    return train_weights, private_prob.astype(float), summary


def precompute_target_encoded_folds(data: dict, train_weights: np.ndarray, n_splits: int) -> list[dict]:
...
def fit_final_model(data: dict, train_weights: np.ndarray, best_params: dict) -> tuple[np.ndarray, dict]:
    X_cv_base = data["X_cv_base"].reset_index(drop=True)
    y_cv = data["y_cv"].reset_index(drop=True)
...
def main() -> None:
    data = load_all_data()
    train_weights, private_prob, adv_summary = compute_raw_odds_weights(data)

    folds = precompute_target_encoded_folds(data, train_weights, N_SPLITS)
    study = optuna.create_study(
        direction="minimize",
        sampler=optuna.samplers.TPESampler(seed=RANDOM_STATE),
        study_name="super_adversarial_optuna",
    )
    ...
    base_private_pred, shapes_info = fit_final_model(data, train_weights, best_params)
    ...
    alpha_010_pred = build_alpha_adjusted_predictions(base_private_pred, private_prob, 0.010)
    alpha_010_summary = make_submission(private_ids, alpha_010_pred, ALPHA_010_SUBMISSION_PATH)

    alpha_020_pred = build_alpha_adjusted_predictions(base_private_pred, private_prob, 0.020)
    alpha_020_summary = make_submission(private_ids, alpha_020_pred, ALPHA_020_SUBMISSION_PATH)
    ...
    summary = {
        ...
        "outputs": [
            ...
            {"kind": "alpha_0.010", "alpha": 0.010, **alpha_010_summary},
            {"kind": "alpha_0.020", "alpha": 0.020, **alpha_020_summary},
        ],
    }
